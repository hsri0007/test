...
import styles from './OrbOutline.module.scss';

interface OrbOutlineProps extends OmitChildren<HTMLMotionProps<'div'>> {
  color: ColorName;
...
export const OrbOutline = (props: OrbOutlineProps) => {
  const {
    className,
    color,
    time,
  ...
  const translateXValueIntro = useTransform(time, [0, introTimingOffset], [200, 0], {
    clamp: true,
    ease: evilEase,
  });
  ...
  const opacityLoop = useTransform(timeLooped, [-1, 0, 1], [0.9, 0.25, 0.9], { clamp: true });
  ...
  return (
    <motion.div
      {...otherProps}
      className={cx(styles.outlineWrapper, className)}
