...
import styles from './virtual-demo.module.scss';
...
const VirtualDemo = async () => {
  const data = await getData();

  return (
    <section className={styles.container}>
      {data.virtualDemoPageHeading && (
        <h1 className={styles.heading}>{data.virtualDemoPageHeading}</h1>
      )}
      ...
      {data.virtualDemoPageSubheading && (
        <p className={styles.subheading}>{data.virtualDemoPageSubheading}</p>
      )}
